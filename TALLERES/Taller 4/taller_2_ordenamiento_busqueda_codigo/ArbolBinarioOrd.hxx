...
template <class T> bool ArbolBinarioOrd<T>::insert(T &val) {

  if (this->raiz == NULL) {
    this->raiz = new NodoBinario<T>(val);
  } else {
    insert(this->raiz, val);
  }
}

template <class T>
bool ArbolBinarioOrd<T>::insert(NodoBinario<T> *n, T &val) {

  if (val < (n->obtenerDato())) {
    if (n->obtenerHijoIzq() != NULL) {

      return insert(n->obtenerHijoIzq(), val);
    ...
    if (n->obtenerHijoDer() != NULL) {
      return insert(n->obtenerHijoDer(), val);
    } else {
...
{
	if(buscar(val)){
	 this->raiz=erase(this->raiz,val);
		return true;
	}
	return false;
}
template <class T>
NodoBinario<T>* ArbolBinarioOrd<T>::erase(NodoBinario<T>* n,T&val)
{
  NodoBinario<T>* temp=NULL;
	if(n->obtenerDato()==val)
	{
  ...
  {
     n->fijarHijoDer(erase(n->obtenerHijoDer(),val));
     return n;
  }
  else if( n->obtenerHijoIzq()->obtenerDato() > val )
  {
    n->fijarHijoIzq(erase(n->obtenerHijoIzq(),val));
    return n;
...
template <class T>
void ArbolBinarioOrd<T>::inordenEnLista(std::list<T>&l)
{
  std::queue<T> q;
  inOrden(q);
  while(!q.empty())
